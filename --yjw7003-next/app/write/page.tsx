...
export default function Write() {
  ...
  const uploadImage = async (e:React.ChangeEvent<HTMLInputElement>) => {
    ...
    const res = await fetch('http://192.168.0.10:33000/api/v1/board/uploadImage', {
      method: 'POST',
      body: formData
    });
    ...
    if (editorRef.current) {
      const img = document.createElement('img');
      img.src = "http://192.168.0.10:33000/" + response?.data?.imageURL;
  ...
  const goWrite = async () => {
    ...
    const res = await fetch('http://192.168.0.10:33000/api/v1/board/regist', {
      method: 'POST',
      body: formData
    });
          ...
          <div className="info-content-container">
            ...
            <div className={styles.fileIconsContainer}>
              <ul className={styles.fileIcons}>
                <li>
                  <img src="/images/file.png" alt="" onClick={prevUploadImage}/>
                </li>
                <li>
                  <img src="/images/paperclip.png" alt="" onClick={prevUploadFile}/>
                </li>
              </ul>
            </div>
...

                      {
                        attachedList.map((item:any,idx) => 
                          <li className={styles.fileInfo} key={idx}>
                            {item?.name}
                            <img src="/images/x.png" alt="" onClick={() => removeFile(idx)}/>
                          </li>
                        )
                      }
